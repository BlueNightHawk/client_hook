...
typedef engine_studio_api_s *(*pfnGetStudioInterface)();

pfnGetStudioInterface pGetStudioInterface;

extern "C" {
void DLLEXPORT HUD_Init_Post();
void DLLEXPORT HUD_DrawNormalTriangles_Post();
...
{
	if (pCurrentInfo && !stricmp(pCurrentInfo->name.c_str(), m_pRenderModel->name))
		return false;

	for (auto & x : nl_viewmodelinfo)
	{
		if (!stricmp(x.second.name.c_str(), m_pRenderModel->name))
		{
			gEngfuncs.Con_Printf("%s %s \n", x.second.name.c_str(), m_pRenderModel->name);
