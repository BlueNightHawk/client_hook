...
typedef struct nlviewmodelinfo_s
{
	int iCameraBone = -1;
	float flAnimScale = 1.0f;

	std::string name;
} nlviewmodelinfo_t;

inline std::map<std::string, nlviewmodelinfo_s> nl_viewmodelinfo;
inline nlviewmodelinfo_s* pCurrentInfo;
...
public:
	...
	bool GetCurrentInfo();
	void CalcBaseAngles();
	void CalcCamAngles(float (*transform)[3][4], int index);

	cl_entity_s view;
};

inline CMiniRenderer g_Renderer;